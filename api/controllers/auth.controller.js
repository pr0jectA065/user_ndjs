...
exports.register = function (req, res) {
    ...
    if (!req.body.fname) {
        return res.status(400).send({
            message: "User first name required for registration.."
        });
    }
    if (!req.body.lname) {
        return res.status(400).send({
            message: "User last name required for registration.."
        });
    }
    if (!req.body.email) {
        return res.status(400).send({
            message: "Email required for registration.."
        });
    }
    if (!req.body.pw) {
        return res.status(400).send({
            message: "Password required for registration.."
        });
    }
    ...
    user.save(function (err) {
        ...
        token.save(function (err) {
            ...
            var data = {
                ...
                html: 'Hello, This is not a plain-text email, I wanted to test some spicy AWS spagetti sauce in NodeJS! <a href="http://0.0.0.0:3030/validate?' + req.params.mail + '">Click here to add your email address to a mailing list</a>'
            }

            request.get(config.notifications.endpoint, (error, response, data) => {
                if (error) {
...
exports.retrieveV0 = function (req, res) {
    ...
    jwt.verify(token, config.secret, function (err, decoded) {
        ...
        User.findById(decoded.id, {
                pw: 0
            }, //add projection to the query to omit password
            function (err, user) {
                if (err) {
                    console.log(err);
